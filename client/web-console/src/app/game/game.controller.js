  ...
  .controller('GameController', [
    ...
    function(userService, unitService, buildingService) {
      ...
      function handleEnemyClick() {
        ...
        if (vm.unitClicked != null && eId != null && eId.search("unit") >= 0) {

          var units = vm.user.units;
          var atkRange = 0;
          for (var i = 0; i < units.length; i++) {
            var u = units[i];
            if (u.id == vm.unitClicked.substr(4)) {
              atkRange = u.atkRange;
              break;
            }
          }
          ...
          for (var j = 0; j < 60; j++) {
            ...
            if (i == 29 && j == 29) {
              ...
              if (grid.isWalkableAt(i, j)) {
                ...
                var terrainType = vm.world[index][coorX - 1][coorY - 1];
      ...
      function stalkEntities() {
        setInterval(function () {
          var promise = userService.listAllVisible();
      ...
      function stalkEmptyWorld() {
        setInterval(function () {
          // top, left corner.
          var initX = vm.panZoom.getPan().x;
          var initY = vm.panZoom.getPan().y;
      ...
      function drawMap(initX, initY) {
        ...
        if (vm.world[index] != null) {
          paint(index, deltaX, deltaY);
        }

        if (vm.world[index] == null && vm.queueDownloadMapChunck.indexOf(index) < 0) {
        ...
        if (!isPainted) {
          ...
          for (var x = 1; x <= 100; x++) {
            ...
            for (var y = 1; y <= 100; y++) {
              ...
              var terrainType = vm.world[index][x - 1][y - 1]; // ohhay!
              group.append("rect")
